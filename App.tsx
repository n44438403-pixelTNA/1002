...
const App: React.FC = () => {
  ...
  const [alertConfig, setAlertConfig] = useState<{isOpen: boolean, message: string, title?: string, type?: string, copyableText?: string, actionText?: string, actionUrl?: string}>({isOpen: false, message: ''});

  // --- POPUP HANDLER ---
  useEffect(() => {
    if (!state.user || !state.settings.adminCustomPopups) return;

    const activePopups = state.settings.adminCustomPopups.filter(p => {
        if (!p.enabled) return false;
        if (p.showTo === 'FREE' && state.user?.subscriptionTier !== 'FREE') return false;
        if (p.showTo === 'PREMIUM' && state.user?.subscriptionTier === 'FREE') return false;
        if (p.class && state.user?.classLevel !== p.class) return false;
        if (p.subject && !Object.keys(state.user?.progress || {}).includes(p.subject)) return false;
        return true;
    });
